...
1. В настройках VNE откройте **Изображения**. Выберите протокол, укажите адрес и ключ, затем нажмите **Подключиться / обновить модели** и выберите модель из списка. Стандартный адрес подсказывается для выбранного протокола. В разделе **Профили подключения** можно сохранить настройки под именем и переключаться между ними; «Новый профиль» позволяет сохранить ещё один набор, а сохранение выбранного обновляет его. Общий стиль не входит в профиль. Это отдельное подключение для картинок; текстовые варианты продолжают использовать прежнее подключение.
