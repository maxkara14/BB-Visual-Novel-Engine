...
1. В настройках VNE откройте **Изображения**. Выберите протокол, адрес, ключ и модель. Это отдельное подключение для картинок; текстовые варианты продолжают использовать прежнее подключение.
...
Форматы запросов адаптированы из BB Comic Forge; его установка не требуется. Списка моделей и проверки подключения отдельной кнопкой пока нет: имя модели вводится вручную. Размеры, качество и возможность работы с референсами зависят от сервиса и модели. Запросы идут из браузера, поэтому сервис должен разрешать CORS. Отказ работы с референсами показывается как ошибка — автоматического повторения без картинок нет.
...
Expand **References** to add up to four PNG/JPEG/WebP images, up to 10 MiB each, or the current avatar. Mark each as appearance or style. References are temporary and disappear when the workshop closes. The four transports are OpenAI Images, OpenAI Chat, Gemini and Naistera, adapted from Comic Forge without a runtime dependency. Enter the model manually; reference support, sizes and quality depend on the service. Direct browser requests require CORS. Failed reference requests are never silently retried without images.
