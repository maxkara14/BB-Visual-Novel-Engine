...
<div style="display: none;" class="bb-vn-data">
  <bb-social-updates>
    <bb-social-update>
      ...
      <user_label>CURRENT_ROLE_OF_USER_TO_CHARACTER</user_label>
      <reason>BRIEF_REASON_FOR_CHANGE</reason>
...
- <user_label> 1-2 words describing {{user}}'s CURRENT relationship label FROM THIS CHARACTER'S POINT OF VIEW (e.g., "dangerous ally", "hidden threat", "trusted friend", "deceptive stranger").
- <user_label> MUST answer: "what has {{user}} become to <name>?" It must NOT answer: "who is <name>?" or "what does <name> feel?"
...
- <user_label> may include {{user}}'s actual persona role/profession ONLY if it describes how <name> sees {{user}}. For example, if {{user}} is truly a guard, "unreliable guard" can be valid. If <name> is a caring aunt, do NOT write "caring aunt"; write what {{user}} became to her instead, such as "beloved nephew", "troublesome ward", or "stubborn child".
- <reason> Short explanation of WHY the NEW impact happened in this turn. Describe the fresh trigger, not the general situation.
- <emotion> 1-2 words describing the character's internal emotional state. If using two nouns, separate with a comma (e.g., "shock, hurt", "joy").`;
...
For EACH action, write a roleplay message from {{user}}'s perspective. The message must actively move the scene, show initiative, and match {{user}}'s persona perfectly. Use the requested output language and obey the active length directive exactly.

CRITICAL RULES FOR EMOTIONAL CHOICE FRAMING:
1. "tone": Describe the emotional flavor of the answer in 1-2 words in the requested output language. Think in placeholder terms like "SHORT_TONE".
...
4. "forecast": A SHORT hint in the requested output language for what this action may cause. Keep it compact: ideally 3-9 words and usually no longer than about 65 characters. Think in placeholder terms like "SHORT_OUTCOME_HINT".
5. "forecast" must logically match the tone and action. Do not write a soft forecast for an aggressive action or vice versa.
6. Prefer vivid, readable tones such as "gently", "boldly", "coldly", "dangerously", "ironically", "vulnerably", "assertively", "affectionately", "harshly" when they fit.
...
9. "risk": OPTIONAL legacy field for backward compatibility. If you include it, use "Low", "Medium", or "High". Do not make it the main focus.
10. "intent": Must be a natural phrase in the requested output language (2-5 words), compact and button-friendly. Prefer roughly 2-4 words and avoid stretching beyond ~42 characters. Never use placeholders, ALL_CAPS tokens, or snake_case.
...
{"options": [
  {
    ...
    "tone": "SHORT_TONE",
    "forecast": "SHORT_OUTCOME_HINT",
    "targets": ["MOST_AFFECTED_CHARACTER"],
    "risk": "OPTIONAL_RISK_LABEL",
    "message": "LONG_ROLEPLAY_REPLY_WITH_ESCAPED_QUOTES_AND_\\n\\n_PARAGRAPHS"
  }
]}
